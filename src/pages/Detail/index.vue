        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 加入购物车点击路由跳转之前，先发送请求添加数据 -->
                <a href="javascript:" @click="addStopCart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    changActive(saleAttrValue, arr) {
      // 排他
      // 遍历全部售卖属性值isChecked为零没有高光
      arr.forEach(item => {
        item.isChecked = 0;
      });
      // 点击的那个售卖属性值添加高亮
    ...
    changeSkuNum(event) {
      // event.target.value为input输入框中的输入值
      // 带有非数字的字符*1=NaN
      let value = event.target.value * 1;
      // 如果用户输入的非法:带有非数字的字符或者值为附属
    ...
    async addStopCart() {
      ...
      try {
        ...
        // 2.成功，路由跳转传送参数
        // 在路由跳转时将产品信息带给下一级路由
        // 简单数据skuNum：路由query参数传递
        // 复杂数据skuInfo，通过会话存储
        // 本地存储、会话存储，一般存储的是字符串
        // 若需要存的是对象，用JSON.stringfy
