            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                ...
                <li :class="{ active: !isOne }" @click="changeOrder('2')">
                  <a href="#">价格<span v-show="!isOne" class="iconfont"
                      :class="{ 'icon-down': isDesc, 'icon-up': !isDesc }"></span></a>
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  computed: {
    ...
    isOne() {
      return this.searchParams.order.charAt(0) === "1";
    },
    isDesc() {
  ...
  methods: {
    // 向服务器发请求获取search模块数据（根据参数的不同返回不同数据进行展示0
    getData() {
      this.$store.dispatch('search/getSearchList', this.searchParams)
    },
    // 删除分类的名字 然后再次发送请求  同时地址栏也要改
    removeCategoryName() {
      // 将参数的值赋值为undefined，该参数就不会带给服务器
  ...
  watch: {
    ...
    $route(newVal, oldval) {
      Object.assign(this.searchParams, this.$route.query, this.$route.params)
      this.getData();
      // 每一次请求完毕，应该把1，2，3分类的id置空，让它接受下一次的1，2，3id
